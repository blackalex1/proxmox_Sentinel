...
if command -v curl &>/dev/null; then
    API_CURL_OPTS=("-fsSL" "-k" "-H" "User-Agent: SentinelController/1.0" "--connect-timeout" "5" "--max-time" "10")
...
req = urllib.request.Request('$URL', headers={'User-Agent': 'SentinelController/1.0'})
try:
