...
def get_ban_center_table(active_bans, banned_keys):
    ...
    rows.append('    <th colspan="3" style="padding: 8px; text-align: center;"><b>🛑 Центр блокировок Aegis IPS</b></th>')
    rows.append('  </tr>')
    
    if not active_bans and not banned_keys:
        rows.append('  <tr>')
        rows.append('    <td colspan="3" style="padding: 8px; color: #a6adc8; text-align: center;"><i>Активных блокировок в системе нет.<br/>Вся сетевая активность находится под контролем Active IPS Engine.</i></td>')
        rows.append('  </tr>')
    else:
        if active_bans:
            rows.append('  <tr style="background-color: #2b2b36; color: #ffffff;">')
            rows.append('    <td colspan="3" style="padding: 6px;"><b>👤 Активные временные блокировки IP</b></td>')
            rows.append('  </tr>')
            rows.append('  <tr style="background-color: #3b3b4f; color: #ffffff;">')
            rows.append('    <td style="padding: 6px; width: 40%;"><b>IP-адрес</b></td>')
            rows.append('    <td style="padding: 6px; width: 30%;"><b>Узел</b></td>')
            rows.append('    <td style="padding: 6px; width: 30%;"><b>Истекает</b></td>')
            rows.append('  </tr>')
        ...
        if banned_keys:
            rows.append('  <tr style="background-color: #2b2b36; color: #ffffff;">')
            rows.append('    <td colspan="3" style="padding: 6px;"><b>🔑 Заблокированные SSH-ключи</b></td>')
            rows.append('  </tr>')
            rows.append('  <tr style="background-color: #3b3b4f; color: #ffffff;">')
            rows.append('    <td style="padding: 6px; width: 35%;"><b>Пользователь</b></td>')
            rows.append('    <td style="padding: 6px; width: 35%;"><b>Узел</b></td>')
            rows.append('    <td style="padding: 6px; width: 30%;"><b>Забанен</b></td>')
            rows.append('  </tr>')
            ...
            for key in banned_keys:
                ...
                rows.append('  <tr>')
                rows.append(f'    <td style="padding: 8px;"><code>{html.escape(key["username"])}</code><br/><small>...{html.escape(short_fp)}</small></td>')
