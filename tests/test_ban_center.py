...
async def test_unban_remote_ip():
    ...
    await execute_write(
        "INSERT INTO temp_bans (server_ip, dst_ip, expire_time) VALUES (?, ?, ?)",
        ("194.87.29.14", "192.168.1.99", "2026-05-25T12:00:00")
    )
    
    mock_task = MagicMock()
    active_remote_blocks[("194.87.29.14", "192.168.1.99")] = mock_task
    
    server = {"ip": "194.87.29.14", "user": "root", "key": "key_path"}
    ...
    with patch('modules.proxmox.monitor.remote.traffic.firewall.run_remote_ssh_cmd', AsyncMock(return_value=(True, "", ""))) as mock_ssh:
        ...
        assert success is True
        assert "Блокировка на VPS 194.87.29.14 снята" in desc
        mock_task.cancel.assert_called_once()
        assert ("194.87.29.14", "192.168.1.99") not in active_remote_blocks
